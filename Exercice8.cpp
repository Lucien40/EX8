...
double V(double const& x, double const& omega, double const& delta) {
  ...
  else if (potential == "step") {
      if (x >= boxrange) {
          return infpot;
      }
      return 0;
  }
  else if (potential == "periodic") {
      return V0 * sin(npot * 2.0 * M_PI * x / (xR - xL));
  }
}
...
int main(int argc, char** argv) {
  ...
  double n = configFile.get<double>("n");
  double k0 = 2. * M_PI * n / (xR - xL);
  double En = 0.5 * M_PI * M_PI * n * n / (4 * boxrange * boxrange);
  double sigma0 = configFile.get<double>("sigma_norm") * (xR - xL);
  double t_detect = configFile.get<double>("t_detect");
  potential = configFile.get<string>("potential");
  boxrange = configFile.get<double>("boxrange");
  infpot = configFile.get<double>("infpot");
  V0 = configFile.get<double>("V0");
  npot = configFile.get<double>("npot");
  string wavetype = configFile.get<string>("wavetype");
  ...
  } else if (wavetype == "eigenstate") {
      for (int i(0); i < Npoints; ++i)
          if (x[i] >= -boxrange && x[i] <= boxrange) {
              psi[i] = sin(n * M_PI * (x[i] - boxrange) / (2. * boxrange));
          } else {
              psi[i] = 0;
          }
  }
